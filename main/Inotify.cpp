...
///////////////////////////////////////////////////////////////////
								...
								if (prevCheckSum != currentChSum) {
									...
									char *envp[] =
									        {
									                "LD_LIBRARY_PATH=../extra/libndef/libndef",
									                NULL };
									char *arguments[] = { "./snep-encode", str,
									        "en-US", NULL };
									pid = fork();
									if (pid == 0) {
										cout << "PID 0" << endl;
									...
									if (!WIFEXITED(status)) {
										printf(
										        "snep-encode terminated abnormally");
										exit(3);
									}
									...
									if (pid2 == 0) {
										...
										perror(
										        "unable to load ./snep-client from libllcp");
										exit(0);
									}
