...
//For custom test
//LD_LIBRARY_PATH=/home/radix/programs/libndef/libndef ./ndef-encode -z ~/programs/libndef/tools/temp/test.txt "en-US" > ndef1
...
//create snep frame
QByteArray createSnepFrame(QList<NDEFRecordList> ndefContainer)
{
#define PUTREQUEST 0x02
	...
	QByteArray lenSet;
	lenSet.reserve(4);
	lenSet.append(ndefLength);
	payload.append(lenSet.rightJustified(4,0));
...
int main(int argc, char *argv[])
{
    ...
    for (int i=1; i<arguments.count(); i++)
    {
        if (arguments.at(i).at(0) == '-')
        {
            ...
            switch (option)
            {
            case 'z': // Personal Text Record
			{
				if ((i+2) >= arguments.size())
				{
					err << "-z option requires two arguments: directory path if file and locale (e.g. \"Hello world\" \"en-US\")" << endl;
					return 1;
				}
				i++;
				qDebug() << "File path : "<< argv[2];
				const QString text = openFile(argv[2]);
...
//    }
    ...
    QByteArray NFCFrame;
    NFCFrame = createSnepFrame(ndef_containers);
    if (output.isOpen ()) {
    	// NDEFMessage contains data
